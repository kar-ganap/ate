# Runbook: Treatment 2b — Autonomous + Discourage Communication

**Treatment**: 2b (Autonomous + Discourage)
**Description**: Agent Teams with autonomous bug assignment, vague prompts, delegate mode ON, 4x2, and **discouraged** inter-agent communication. Paired with Treatment 2a (encourage) for the communication comparison.
...
for bug in 20945 18654 7847 4384 19301 22221 22528 22494; do
  uv run ate run scaffold 2b $bug
done
```
...
Work independently. Do not message other agents. Report findings only to the team lead.

Budget ~25 tool calls per bug before reporting. After fixing each bug, save your patch by running `git -C data/ruff diff > data/patches/treatment-2b/bug-{id}.patch` (replacing `{id}` with the bug number), then reset the source with `git -C data/ruff checkout .` before starting the next bug.
````
...
### Communication is KEY data for this treatment (inverse of 2a)

Treatment 2b explicitly discourages communication. Watch for:

- **Do agents actually stay silent?** (They should — that's the guidance)
- **Any messages despite the guidance?** (This is notable if it happens)
- **Does isolation hurt quality?** (Compare with 2a results later)
- **Does the lead synthesize at the end?**

Record communication observations in notes.md:

```markdown
## Communication Patterns
- Total inter-agent messages observed: ___ (should be 0 or near 0)
- Any violations of "discourage" guidance: yes/no (details: ___)
- Did agents report only to lead as instructed: yes/no
```
...
| Signal | Action |
|--------|--------|
| New filenames, different errors | Progress |
| Same files/errors 3+ times | Prepare to nudge |
| Agent messaging another agent | **Note this!** Violation of guidance |
| Agent reporting to lead only | Expected behavior |
...
The whole point of this treatment is to observe what "discourage" produces naturally. Do not prompt agents to communicate. Only nudge on technical stuck-ness.
...
wc -l data/patches/treatment-2b/*.patch 2>/dev/null

# If aggregate patch needed:
git -C data/ruff diff --stat
git -C data/ruff diff > data/patches/treatment-2b/remaining.patch
```
...
Update `data/transcripts/treatment-2b/bug-{id}/metadata.json` for each bug.
...
- [ ] Communication count (should be 0 or near 0)
- [ ] Whether agents followed "discourage" guidance
- [ ] Per-bug outcomes
...
- [ ] Communication silence documented
- [ ] "Discourage" compliance noted
